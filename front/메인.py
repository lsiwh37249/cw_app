...
df["검수 종료일"] = pd.to_datetime(df["검수 종료일"], format='ISO8601').dt.date
...
FROM df
WHERE "검수 종료일" >= '{start_date}' AND "검수 종료일" <= '{end_date}'
AND "작업 상태" IN ('ALL_FINISHED', 'CHECK_END')
GROUP BY "검수 종료일"
ORDER BY "검수 종료일" ASC
"""
result = duckdb.query(query).to_df()
result["검수 종료일"]= result["검수 종료일"].astype(str)
